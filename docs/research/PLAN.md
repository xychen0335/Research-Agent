...
分开保存任务数据、SFT 教师轨迹和 RL 在线轨迹。任务包含公开问题、环境入口与预算，以及仅供评分器读取的答案、别名、事实要点和参考证据。SFT 使用筛选后的教师示范；RL 由当前策略实际调用工具产生轨迹，不能用固定教师记录冒充在线交互。
...
教师在同一 harness 中执行任务，筛选答案正确且调用有效的 SFT 轨迹，保留适量先失败再恢复的成功示范。生成轨迹的教师不读取隐藏答案，离线评分器负责筛选。保存教师版本、采样参数与原始事件，确保观测来自工具执行。
