...
python -c "from research_agent.training.compat import probe; r=probe(); assert r.get('cuda_available'), 'gpu_node.sh requires CUDA'; assert r.get('qwen35_4b_cached'), 'missing models/Qwen3.5-4B; copy the HF snapshot into the working directory'; print('cuda_ok')"
